...
public class OrderPackageDAO {
    ...
    private static final String SELECT_READY_PACKAGES_SQL =
    ...
    private static final String SELECT_ITEMS_BY_PACKAGE_SQL =
        "SELECT ITEM_ID, PACKAGE_ID, PROFIL_BLOC_ID, DATA_TYPE, OBIS_CODE, POD_ID, STATUS, CREATED_TIMESTAMP " +
        "FROM SMC_ORDER_ITEMS " +
        "WHERE PACKAGE_ID = ?";

    // SQL for updating package status
    private static final String UPDATE_PACKAGE_STATUS_SQL =
        "UPDATE SMC_ORDER_PACKAGES SET STATUS = ? WHERE PACKAGE_ID = ?";

    // SQL for marking items as processed
    private static final String UPDATE_ITEM_STATUS_SQL =
        "UPDATE SMC_ORDER_ITEMS SET STATUS = ? WHERE PACKAGE_ID = ?";



    private DataSource dataSource;

    private static final String SELECT_INTERVALS_SQL =
        "SELECT " +
    ...
    private static final String CALL_ADD_ORDER_ITEM_SP =
        "{CALL SP_SMC_ADD_ORDER_ITEM(?, ?, ?, ?, ?, ?, ?)}"; // <-- Now 7 parameters
     ...
     */
    public long createOrderItem(String channelId, String profilBlocId, String dataType, String obisCode, String podId, String rawXml) throws SQLException {
        try (Connection conn = dataSource.getConnection();
    ...
    private static final String CHECK_CHANNEL_RELEVANT_SQL =
        "SELECT 1 FROM SMC_PUSH_RELEVANT_CHANNELS WHERE CHANNEL_ID = ? AND IS_RELEVANT = 1";

    /**
     * Checks if a given channel ID is marked as "push relevant" in the configuration table.
     *
